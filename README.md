...
## 1. 리눅스 명령어 : kill, top, job, ps  명령어 조사하기
...
- NI: nice 값, 일의 nice value값이다. 마이너스를 가지는 nice vlaue는 우선순위가 높음
- VIRT: 가상 메모리의 사용량
...
#### Top 명령어 옵션
- shfit+p: cpu 사용률이 높은 프로세스 순서대로 표시
- shift+m: 메모리의 사용률이 높은 프로세스 순서대로 표시
- shift+t: 프로세스가 돌아가고 있는 시간 순서대로 표시
- K: 프로세스 kill-k 입력 후 종료할 pid 입력 signal 을 입력하라고 하면 kill signal인 9를 입력
- A: 메모리 사용량에 따라 정렬
- B: batch 모드 작동
- C: 명령행/프로그램 이름 토글
